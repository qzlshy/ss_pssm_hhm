...
for i in range(len(hmm_list)):
	for j in range(len(hmm_list[i])):
		hmm_rsp.append(hmm_list[i][j])

cnt_hmm=np.mean(hmm_rsp)
std_hmm=np.std(hmm_rsp)
...
def g_data(n):
	...
	noise=np.abs(np.random.normal(loc=1.0, scale=0.5, size=(1,l,1,57)))
	data=data*noise
...
print("train_num:",len(seq))

init = tf.global_variables_initializer()

num=list(range(len(seq)))
saver = tf.train.Saver(max_to_keep=0)

cw=1
kf = KFold(n_splits=10)
with tf.Session() as sess:
	for train, test in kf.split(seq):
		print('kf:',cw)
		while 1:
			...
			for epoch in range(training_epochs):
				...
				for i in range(len(train)):
					if len(seq[train[i]])<=20:
						continue
					batch_x,batch_y,batch_z=g_data(train[i])
				...
				for i in range(len(test)):
					batch_x,batch_y,batch_z=g_data_test(test[i])
